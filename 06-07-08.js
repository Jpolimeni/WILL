...
function crearClasePersona() {
  class Persona {
    ...
    getFriends() {
      ...
      // Tu código aca:
      let newArray = this.amigos.map(function(obj){
        return(obj['nombre'])
      })
      return(newArray)
    }
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:
      let edadesSumadas = 0
      let edades = this.amigos.map(function(obj){
        return(obj['edad'])
      })
      for(let i=0; i<edades.length; i++){
        edadesSumadas = edadesSumadas + edades[i]
      }
      return(edadesSumadas/edades.length)
    }
  };
